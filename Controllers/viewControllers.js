...
const getView = asyncHandler(async (req, res) => {
  ...
  const Product1 = await product.find({ postedBy: id });
  ...
  const Product2 = await product.find({ postedBy: id });
  // console.log(Product1.view);
  ...
  const ProductStat = await product.find({ postedBy: id });
  // console.log(Product1.view);
  ...
  const Product = await product.find({ postedBy:id });
  const createDateUser = await user.findById(id);
